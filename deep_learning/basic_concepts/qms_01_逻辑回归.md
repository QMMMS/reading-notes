...
图示为三分类中单个样本的交叉熵损失计算过程

### 极大似然解释

> 复习一下，极大似然的思想是，想找到一个估计，使**已经观测到的事件发生的可能性最大**

另一种解释是极大似然。我们将神经网络的每个类别输出视作概率，label 是实验的观测值，每个样本是一次独立抽样。让模型输出和 label 相似最大则为最优模型。

一次抽样模型预测正确的概率是 $$\prod_M f(x_{ij})^{y_{ij}}$$，对于上面的例子就是 $$0.8^1*0.1^0*0.1^0$$，对于n次抽样为 $$\prod_N \prod_M f(x_{ij})^{y_{ij}}$$，让这组概率发生概率最大的模型就是我们要找的模型，记得加一个负号求最小值：
$$
\arg \max \prod_N \prod_M f(x_{ij})^{y_{ij}} \to \arg \max - \sum_N \sum_M {y_{ij}} \log f(x_{ij})
$$
